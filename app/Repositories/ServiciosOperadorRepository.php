...
class ServiciosOperadorRepository extends BaseRepository
    ...
    public function getServiciosidUsuario($id_usuario)
        ...
        {
        return DB::table('usuario_servicios')->join('usuario_operadores', 'usuario_servicios.id_usuario_operador', '=', 'usuario_operadores.id_usuario_op')->join('users', 'usuario_operadores.id_usuario', '=', 'users.id')->where('users.id', $id_usuario)->where('estado_servicio', '=', 1)->get();
        }
    ...
    *      */
    public function storeUpdateSerchEngine($objeto, $tipo, $id_tipo, $search)
        ...
        foreach($objeto as $servicioBase)
            ...
            $operadorData = $operador::where('id_tipo', $id_tipo)->where('tipo_busqueda', '=', $tipo)->update(['search' => $search]);
            }
        return true;
        }
    public function UpdateGeoLoc($inputs)
        ...
        {
        if (isset($inputs['id_provincia'])) $id = $inputs['id_provincia'];
        if (isset($inputs['id_canton'])) $id = $inputs['id_canton'];
        if (isset($inputs['id_parroquia'])) $id = $inputs['id_parroquia'];
    ...
    public function updatePromo($promo)
        ...
        $operadorData = $operador::where('id', $promo['id'])->update([$campo => $input[$campo]]);
        }
    ...
    public function updateServDetItiner($input, $campo)
        ...
        $operadorData = $operador::where('id', $input['id'])->update([$campo => $input[$campo]]);
        }
    // Realiza la logica del update
    ...
    public function updateServ($input, $campo)
        ...
        $operadorData = $operador::where('id', $input['id_usuario_servicio'])->update([$campo => $input[$campo]]);
        }
    // Entrega el arreglo de Servicios por operador
    public function getServiciosOperador($id_usuario_operador)
        ...
        {
        $user_servicios = new $this->model;
        return $user_servicios::where('id_usuario_operador', $id_usuario_operador)->where('estado_servicio', '=', 1)->get();
        }
    // Entrega el arreglo de Servicios por operador
    ...
    public function getImagePromocionesOperador($tipo, $id_promocion)
        ...
        return $promociones::where('id_auxiliar', $id_promocion)->where('id_catalogo_fotografia', '=', $tipo)->where('estado_fotografia', '=', 1)->get();
        }
    // Entrega el arreglo de Imagenes por promocion por operador
    public function getGenericImagePromocionesOperador($tipo, $idtipo)
        ...
        {
        $promociones = new $this->image;
        return $promociones::where('id_auxiliar', $idtipo)->where('id_catalogo_fotografia', '=', $tipo)->where('estado_fotografia', '=', 1)->get();
        }
    // Entrega el arreglo de Imagenes por promocion por operador
    public function getImageUbcacionGeografica($id, $tipo)
        ...
        {
        $promociones = new $this->image;
        return $promociones::where('id_auxiliar', $id)->where('id_catalogo_fotografia', '=', $tipo)->where('estado_fotografia', '=', 1)->get();
        }
    // Entrega el arreglo de Imagenes por promocion por operador
    public function getImageOperador($id_aux, $catalogo)
        ...
        {
        $promociones = new $this->image;
        return $promociones::where('id_auxiliar', $id_aux)->where('id_catalogo_fotografia', '=', $catalogo)->where('estado_fotografia', '=', 1)->get();
        }
    // Entrega el arreglo de Itinerarios por operador
    public function getItinerario($id_itinerario)

        {
        return DB::table('itinerarios_usuario_servicios')->where('id', '=', $id_itinerario)->get();
        }
    // Entrega el arreglo de Itinerarios por operador
    ...
    public function getEvento($id_evento)
        ...
        {
        return DB::table('eventos_usuario_servicios')->where('id', '=', $id_evento)->get();
        }
    // Entrega el arreglo de Detalle Itinerarios por operador
    public function getDetalleItinerario($id_detalle_itinerario)

        {
        return DB::table('detalles_itinerario')->where('id', '=', $id_detalle_itinerario)->get();
        }
    // Entrega el arreglo de Itinerarios por id
    public function getPromocion($id_promocion)

        {
        return DB::table('promocion_usuario_servicio')->where('id', '=', $id_promocion)->get();
        }
    ...
    public function getServiciosOperadorUnicos($id_usuario_operador)
        ...
        {
        return DB::table('usuario_servicios')->join('catalogo_servicios', 'usuario_servicios.id_catalogo_servicio', '=', 'catalogo_servicios.id_catalogo_servicios')->where('id_usuario_operador', $id_usuario_operador)->where('estado_servicio', '=', 1)->groupby('usuario_servicios.id_catalogo_servicio')->distinct()->select('catalogo_servicios.nombre_servicio', 'catalogo_servicios.id_catalogo_servicios', 'usuario_servicios.id', 'usuario_servicios.observaciones', 'usuario_servicios.estado_servicio_usuario', 'usuario_servicios.id_usuario_operador')->get();
        }
    public function getServiciosOperadorAll($id_usuario_operador)

        {
        $list = DB::table('usuario_servicios')->join('catalogo_servicios', 'usuario_servicios.id_catalogo_servicio', '=', 'catalogo_servicios.id_catalogo_servicios')->where('id_usuario_operador', $id_usuario_operador)->where('estado_servicio', '=', 1)->select('usuario_servicios.nombre_servicio', 'usuario_servicios.detalle_servicio', 'catalogo_servicios.id_catalogo_servicios', 'usuario_servicios.id', 'usuario_servicios.estado_servicio_usuario', 'usuario_servicios.id_usuario_operador')->paginate(10);
        foreach($list->items() as $value)
            {
            $dataImage = DB::table('images')->where('id_usuario_servicio', '=', $value->id)->where('profile_pic', '=', 1)->where('estado_fotografia', '=', 1)->where('id_catalogo_fotografia', '=', 1)->select('filename')->get();
            if (count($dataImage) > 0)
                {
                $value->filename = $dataImage[0]->filename;
                }
              else
                {
    ...
    public function getEventosporId($id)
        ...
        {
        return DB::table('eventos_usuario_servicios')->where('id', $id)->get();
        }
    ...
    public function getServiciosOperadorporIdServicio($id_usuario_operador, $id_catalogo)
        ...
        {
        $user_servicio = Usuario_Servicio::where('id_usuario_operador', '=', $id_usuario_operador)->where('id_catalogo_servicio', '=', $id_catalogo)->get();
    ...
    public function getPermiso($id)
        ...
        {
        $data = DB::table('usuario_servicios')->join('usuario_operadores', 'usuario_servicios.id_usuario_operador', '=', 'usuario_operadores.id_usuario_op')->where('usuario_servicios.id', $id)->select('usuario_operadores.id_usuario_op', 'usuario_operadores.id_usuario')->first();
        return $data;
        }
    public function getPermisoPromocion($id)

        {
        return DB::table('usuario_servicios')->join('promocion_usuario_servicio', 'usuario_servicios.id', '=', 'promocion_usuario_servicio.id_usuario_servicio')->where('promocion_usuario_servicio.id', $id)->select('promocion_usuario_servicio.id_usuario_servicio')->first();
        }
    public function getPermisoEvento($id)

        {
        return DB::table('usuario_servicios')->join('eventos_usuario_servicios', 'usuario_servicios.id', '=', 'eventos_usuario_servicios.id_usuario_servicio')->where('eventos_usuario_servicios.id', $id)->select('eventos_usuario_servicios.id_usuario_servicio')->first();
        }
    public function getPermisoItinerario($id)

        {
        return DB::table('usuario_servicios')->join('itinerarios_usuario_servicios', 'usuario_servicios.id', '=', 'itinerarios_usuario_servicios.id_usuario_servicio')->where('itinerarios_usuario_servicios.id', $id)->select('itinerarios_usuario_servicios.id_usuario_servicio')->first();
        }
    /*Booking*/
    // **********************************************************************************//
    ...
    // Realiza la logica del update
    public function updateServBooking($input, $campo)

        {
        $operadorData = DB::table('booking_abcalendar_calendars')->where('id', $input['id'])->update([$campo => $input[$campo]]);
        /*$operadorData = DB::table('booking_abcalendar_calendars')
        ->where('id', $input['id'])
    ...
    public function getIDUsuario($id)
        ...
        {
        return DB::table('usuario_operadores')->select('id_usuario')->where('id_usuario_op', $id)->distinct()->get();
        }
    // **************************************************************************//
    //           DEVUELVE EL ID DEL USUARIO DEL OPERADOR                        //
    // **************************************************************************//
    ...
    public function ingresarUsuario($id, $email, $password, $username, $fecha)
        ...
        {
        return DB::table('booking_abcalendar_users')->insert(['id' => $id, 'role_id' => '3', 'email' => $email, 'password' => '', 'name' => $username, 'created' => $fecha, 'status' => 'T', 'is_active' => 'T', 'ip' => '::1']);
        }
    // **************************************************************************//
    //           DEVUELVE EL ID DEL USUARIO DEL OPERADOR                        //
    // **************************************************************************//
    ...
    public function getControlDashboard($id_usuario_operador)
        ...
        {
        return DB::table('control_dahsboards')->select(DB::raw('id_usuario_operador,catalogo_servicio,cantidad'))->where('id_usuario_operador', $id_usuario_operador)->where('estado', '=', 1)->get();
        }
    // Guarda las promociones por usuario_servicio
    ...
    public function storeUpdatePermanentePromocion($permanente, $id)
        ...
        {
        $operador = new $this->promocion;
        $operadorData = $operador::where('id', $id)->update(['permanente' => $permanente]);
        return $operadorData;
        }
    ...
    public function getPostsUsuarioServicio($id_usuario_servicio)
        ...
        {
        $posts = new $this->post;
        $arrayPosts = $posts::where('id_usuario_servicio', '=', $id_usuario_servicio)->where('status', 1)->orderBy('updated_at', 'DESC')->get();
        return $arrayPosts;
        }
    ...
    public function postDetailsById($idPost)
        ...
        $postUpdate->save();
        $details = $posts::where('id', '=', $idPost)->where('status', 1)->first();
        return $details;
        }
    ...
    public function lastPostCreated($id_usuario_servicio = null, $data, $limit = 5)
        ...
        if ($id_usuario_servicio != null)
            {
            $recentPost = $this->post->where('status', 1)->where('id_usuario_servicio', $id_usuario_servicio)->orderBy('created_at', 'DESC')->limit($limit)->get();
            }
          else
            {
            $recentPost = $this->post->where('status', 1)->join('usuario_operadores', 'id_operador', '=', 'id_usuario_op')->orderBy('posts.created_at', 'DESC')->limit($limit)->get();
            }
        return $recentPost;
        }
    public function popularPost($id_usuario_servicio, $data, $limit = 5)
        ...
        {
        $recentPost = $this->post->where('status', 1)->where('id_usuario_servicio', $id_usuario_servicio)->orderBy('views', 'DESC')->limit($limit)->get();
        return $recentPost;
        }
    }